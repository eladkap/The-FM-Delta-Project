...
#define MAX_BLOCK_SIZE 8 // max size of block in bytes
...
void setBitmap(Packet file1, Packet file2, int* bitmap, int bitmap_size, int block_size);
...
int deltaEncode(Packet file1, Packet file2, Packet* delta, int* bitmap, int bitmap_size, int block_size);
...
void setBitmap(Packet file1, Packet file2, int* bitmap, int bitmap_size, int block_size){
	...
	char block_value1[MAX_BLOCK_SIZE];
	char block_value2[MAX_BLOCK_SIZE];
	int blocks_count=0;
	...
	while(i<len1 && j<len2){
		for (int b=0; b<block_size; b++){
			...
			block_value1[b]=file1.array[i];
			i++;
			block_value2[b]=file2.array[j];
			j++;
		}
		
		blocks_count=0;
		for (int b=0; b<block_size; b++){
			if (block_value1[b]==block_value2[b]){
				blocks_count++;
			}
		}

		// blocks are identical
		if (blocks_count==block_size){
...
int deltaEncode(Packet file1, Packet file2, Packet* delta, int* bitmap, int bitmap_size, int block_size){
	...
	while(i<len1 && j<len2){
		...
		if (blocks_count!=block_size){
			for (int b=0; b<block_size; b++){
				delta->array[delta_index++]=block_value2[b]; // writing byte to delta file
			}
		}
		bitmap_index++;
		//i++;
...
/* ./delta_encoding -e <packets_file> <compressed_file> <packets_number> <block_size> */
/* ./delta_encoding -d <compressed_file> <source_file> <packets_number> <block_size> */
...
int main(int argc, char* argv[]){
	...
	int bitmap_size=calcBitmapSizeInBits(10,12,block_size);
	printf("bitmap size: %d\n",bitmap_size);

	int* bitmap=(int*)malloc(sizeof(int)*bitmap_size);

	setBitmap(p1,p2,bitmap,bitmap_size,block_size);

	showBitmap(bitmap,bitmap_size);
	
	Packet delta;

	deltaEncode(p1,  p2,  &delta,  bitmap, bitmap_size,block_size);

	printf("delta len=%d\n",delta.length);
	for (int i=0; i<delta.length; i++){
		printf("%c",delta.array[i]);
	}
	printf("\n");
	*/
	/*test*/
	...
	if (strcmp(argv[1],"-e")==0){
		...
		for (int p=0; p<packets_num-1; p++){
			...
			setBitmap(packets[p],packets[p+1],bitmap,bitmap_size,block_size);

			//showBitmap(bitmap,bitmap_size);

			deltaEncode(packets[p],packets[p+1],&deltas[p+1],bitmap,bitmap_size,block_size);
